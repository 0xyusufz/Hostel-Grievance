...
9. **Fix #2 Login brute via spoofed IP** `Attacker -> X-Forwarded-For: 1.1.1.$i / X-Real-IP -> POST /api/login brute` -> `5/15m` bypass -> mitigate `src/server/http/ip.ts` `getClientIp` direct `c.env.incoming.socket.remoteAddress` when `TRUST_PROXY=false/unset` (ignore `X-Forwarded-For`, `X-Real-IP`, `Forwarded`), explicit `TRUST_PROXY=true` only behind verified proxy (validated first IP `isIP()`). **Direct:** `Internet -> Node -> remoteAddress` **Trusted proxy:** `Internet -> Trusted Proxy (overwrite XFF) -> Node -> validated XFF` **Misconfigured:** `Internet -> Node TRUST_PROXY=true -> spoofable` **must not use**. Verify `vitest FIX #2 TEST 1-4` rotation `429`, `TRUST_PROXY true` `1.1.1.1` vs `1.1.1.2` isolation, `not-an-ip` fallback. Residual: proxy must overwrite `XFF`.
10. **Fix #3 — Security visibility** `Attacker -> failed login / BOLA 403 / 429 spam / oversized attachment -> 401/403/429/400` blocked but invisible → mitigate `src/server/http/audit.ts` `JSON` `timestamp,event,userId,clientIp,method,path,resourceId` to `stdout` via `getClientIp()` (no `X-Forwarded-For` direct). **Flow:** `Browser -> API security control (401/403/429/400) -> audit -> stdout -> admin investigation`. **Trust:** `Browser -> API -> getClientIp -> audit` `JSON` mitigates `log injection` (`\n` escaped). Blocked attacks remain blocked by `auth`/`authz`/`rateLimit`/`sniff`; logging does **not** replace controls. **Residual:** `stdout` persistence/rotation depends on deployment, no SIEM.
